...
3. Переходим через терминал в папку `resources` и создаём через терминал виртуальное окружение командой
   ```
   python -m venv .venv
   ```

4. Запускаем виртуальное окружение через терминал командой:
   ```
   .venv\Scripts\activate 
   ```

5. После активации виртуального окружения, устанавливаем необходимые библиотеки следующей командой:
   ```
   pip install -r requirements.txt
   ```
   
6. Через терминал (всё ещё из виртуального окружения) переходим в папку `scripts` и запускаем скрипт переноса данных в БД командой:
   ```
...
7. После успешного переноса подключаемся к БД через DBeaver или pgAdmin, чтобы проверить данные в таблице (имя пользователя и пароль - postgres)
...
Jupyter Notebook можно запустить через различные IDE или программы для работы с этим форматом; в моём случае я использовал VS Code с расширением Jupyter. Также для ноутбука нужно будет указать ядро выполнения python из папки `.venv` (нужно будет дополнительно подгрузить в виртуальное окружение файлы ядра, VS Code предлагает это сделать сразу же)
